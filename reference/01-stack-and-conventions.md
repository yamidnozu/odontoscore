...
2. **One self-contained `<script type="module">` island + import map** — ONLY
   for the three.js feature (bare `"three"` specifiers), mapped to LOCAL
   vendored files. The island must be optional: if modules fail, the rest of
   the page still works.

Consequence: tool pages are previewed via `python -m http.server`, never
`file://`. Full rules and pins: `14-library-pinning.md`.
