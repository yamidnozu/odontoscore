...
## TIER E — Hard-won lessons from real affiliate builds (v2)

Every item below is a bug that actually shipped and was fixed during a live
e-bike build. They are cheap to prevent and expensive to debug.

### E.1 ❌ Inline `<svg>` icons with no width/height explode to full column width

**Symptom:** a check/✕ icon in a section heading (pros/cons, "verified", spec
chips) renders **enormous** — filling the whole card — instead of ~18px. The
list icons below it are fine.

**Root cause:** an inline SVG that has a `viewBox` but **no `width`/`height`**
is a replaced element with a default intrinsic size of 300×150; with
`max-width:100%` it stretches to the parent's width. A CSS rule that sizes
`.card li svg` does NOT cover the `.card h3 svg` or `.chip svg`.

**Fix:** size EVERY inline-icon context explicitly. Don't rely on one rule.
```css
.pc-col h3 svg, .review-verified svg, .card-spec svg, .hero-badge svg,
.feat-ico svg { width: 1.15em; height: 1.15em; flex: none; }
```
Rule of thumb: any element whose text you set from `IC[...]` needs a matching
`… svg { width/height }`. Grep your icon map usages and confirm each is sized.

### E.2 ❌ A button inside a nav/link gets its color from `.nav a`, not `.btn`

**Symptom:** the "Comparador" CTA in the header is a lime button but its text is
a pale grey — unreadable. On hover it turns white — worse.

**Root cause:** CSS specificity. `.nav-links a` (class + element = 0,1,1) beats
`.btn-primary` (class = 0,1,0). So the generic nav-link color/hover overrides
the button's own color AND background.

**Fix:** give the in-nav button a rule at least as specific, covering `:hover`:
```css
.nav-links a.nav-cta, .nav-links a.nav-cta:hover {
  color: var(--accent-ink); background: var(--accent);
}
```
General principle: when a `.btn` lives inside a styled `a`/`li` context, re-assert
its color/background with the ancestor in the selector. Also make count/badge
pills contrast with their host (a lime badge on a lime button is invisible — use
the ink color for the badge background).

### E.3 ❌ Overhang "close/remove" badges clipped by an `overflow` ancestor

**Symptom:** a small ✕ badge meant to stick out past the top-right corner of a
thumbnail looks sliced off exactly at the thumbnail's edge.

**Root cause:** an ancestor clips it — either the thumbnail itself has
`overflow:hidden` (to round the image), or the scroll row has `overflow-x:auto`.
And remember the spec trap: **`overflow-x:auto` forces `overflow-y` to compute to
`auto` too** (B.1), so a horizontally-scrolling row also clips vertically.

**Fix:** the badge and the corner it hangs off must have NO clipping ancestor
between them.
- Wrap the image in its own `overflow:hidden` box, and make the badge a *sibling*
  of that box inside a `position:relative` non-clipping wrapper.
- If the row scrolls, and you have few items (e.g. ≤4 compare slots that always
  fit), drop `overflow-x:auto` entirely (`overflow:visible`) and add small top/
  right padding so the overhang lives inside the padding box.
```html
<div class="slot-wrap"><!-- position:relative, overflow visible -->
  <div class="thumb"><img></div><!-- overflow:hidden here only -->
  <button class="remove-badge">×</button><!-- top:-8px; right:-8px -->
</div>
```

### E.4 ❌ IntersectionObserver lazy-init never fires when the tab isn't composited

**Symptom:** a map / heavy widget you lazy-mount via IntersectionObserver stays
blank in an automated/hidden preview tab (and sometimes in background tabs),
even after scrolling it into view programmatically. Manual init works.

**Root cause:** IntersectionObserver depends on the rendering/compositing
pipeline. A pane that isn't being displayed/painted may never deliver
intersection callbacks. Your init that's gated behind "when it becomes visible"
therefore never runs.

**Fix:** don't gate *critical* mounts solely on IO. Use a short `setTimeout`
(or `requestIdleCallback` with a timeout) so the widget initializes regardless
of visibility; keep it idempotent so it runs once.
```js
function start(){ if (started) return; started = true; mount(); }
setTimeout(start, 350);   // fires even if IO never does
```
Keep IO only for truly optional/expensive deferral, never for "does it appear at
all". This also makes the feature testable in headless preview.

### E.5 ❌ Chasing a "cached" bug: local preview serves stale HTML/JS

**Symptom:** you edit `main.js`, reload the local preview, and the change isn't
there — you burn time debugging code that's already correct.

**Root cause:** `python -m http.server` sends no cache headers, so the browser
caches `main.js?v=YYYYMMDD` and the HTML page itself. If you keep the same `?v=`,
the browser serves the old file. (The Hostinger `.htaccess` fixes this in prod —
locally it bites.)

**Fix:**
- **Bump the `?v=` cache-buster on EVERY rebuild**, not just deploys. If you
  generate pages, regenerate them all so the new `?v=` propagates to every page.
- When iterating locally, hard-reload or append a throwaway query to the page URL
  (`index.html?cb=123`) to force fresh HTML that then references the fresh assets.
- Before concluding "my fix didn't work", confirm the browser actually fetched
  the new file (DevTools Network → 200 with new size, or check a marker string
  via `fetch()` in the console).

### E.6 ❌ Web maps: wheel over the map scrolls the page instead of zooming

**Symptom:** the user hovers the embedded map and spins the wheel expecting to
zoom; the page scrolls instead.

**Root cause:** the map was initialised with wheel-zoom disabled (e.g. Leaflet
`scrollWheelZoom:false`).

**Fix:** enable it (`scrollWheelZoom:true`). Wheel-over-map then zooms the map;
wheel elsewhere scrolls the page as normal. Accept the mild "scroll trap" when the
cursor sits on the map — it's the behavior users expect from a map. See
`14-mapa-autonomia.md`.

### E.7 ✅ The build-from-data generator is the reliable way to satisfy BOTH invariants

The two invariants "pages generated from data" and "core content renders without
JS" pull in opposite directions if you render fichas at runtime with JS. The
clean resolution used in production: a small **build script** (`tools/build_site.py`,
Python) that reads `productos.json` and **writes one static HTML file per record**
with the core facts (name, price, buy button, specs, disclosure) hardcoded into
the HTML, plus `data-*` hooks and a generated `lib/db.js` for the JS to enrich
(charts, comparator, map). Result: SEO-perfect, works with JS off, and no product
is hand-written — adding a product = editing the JSON and re-running the build.
See `05-pages-and-features.md` §Generator.
